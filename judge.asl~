...
//A PARTIR DE AQUI HAY QUE AÑADIR LOS NUEVOS PATRONES RELATIVOS 
//A LOS NUEVOS INTERCAMBIOS ENTRE FICHAS ESPECIALES

//AQUI NO SE SI A�ADIR TIPO Y QUEDARIA LA FUNCION ASI!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
comprobarPatronesEsp(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern):-
	((patternCoCt(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "CoCt")|
	(patternGsCt(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "GsCt")|
	(patternIpCt(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "IpCt")|
	(patternIpCo(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "IpCo") |
	(patternCoGs(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "CoGs") |
	(patternIpGs(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "IpGs") |
	(patternCt2(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "Ct2") |
	(patternCo2(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "Co2") |
	(patternGs2(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "Gs2") |
	(patternIp2(Color,X1,Y1,X2,Y2,Tipo1,Tipo2,Pattern) & Pattern = "Ip2")).

	
	//patternCoCt
	patternCoCt(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	((Tipo1 = "Co" & Tipo2 = "Ct") | ( Tipo2 = "Co" & Tipo1 = "Ct")).
	
	//patternGsCt
	patternGsCt(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	((Tipo1 = "Gs" & Tipo2 = "Ct") | ( Tipo2 = "Gs" & Tipo1 = "Ct")).
	
	//patternIpCt
	patternIpCt(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	((Tipo1 = "Ip" & Tipo2 = "Ct") | ( Tipo2 = "Ip" & Tipo1 = "Ct")).
	
	//patternIpCo
	patternIpCo(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	((Tipo1 = "Co" & Tipo2 = "Ip") | ( Tipo2 = "Co" & Tipo1 = "Ip")).
	
	//patternCoGs
	patternCoGs(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	((Tipo1 = "Co" & Tipo2 = "Gs") | ( Tipo2 = "Co" & Tipo1 = "Gs")).
	
	//patternIpGs
	patternIpGs(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	((Tipo1 = "Ip" & Tipo2 = "Gs") | ( Tipo2 = "Ip" & Tipo1 = "Gs")).
	
	//patternCt2
	patternCt2(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	Tipo1 = "Ct" & 
	Tipo2 = Tipo1.
	...
	//patternCo2
	patternCo2(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	Tipo1 = "Co" & 
	Tipo2 = Tipo1.
	...
	//patternGs2
	patternGs2(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	Tipo1 = "Gs" & 
	Tipo2 = Tipo1.
	...
	//patternIp2
	patternIp2(Color,X1,Y1,X2,Y2,Tipo1,Tipo2):-
	tablero(celda(X1,Y1),ficha(Color,Tipo1)) &
	tablero(celda(X2,Y2),ficha(Color,Tipo2)) &
	Tipo1 = "Ip" & 
	Tipo2 = Tipo1.
...
+moverDesdeEnDireccion(pos(X,Y),Dir)[source(P)] :
	not turnoActual(P) & not fueraTurno(P,N) <- 
...
/*
		...
		Necesitareis nuevas reglas para intercambiarFichas(X,Y,Dir,P) para detectar los intercambios de fichas especiales 
		y realizar los cambios adecuados en el tablero y las invocaciones oportunas. 

*/


+intercambiarFichas(X1,Y1,Dir,P) : 
	nextPosition(X1,Y1,Dir,X2,Y2) & plNumb(P,PlNumb) & 
...
+!patternMatchPosition(I,J): 	tablero(celda(I,J,_),ficha(C,_)) & 
								comprobarPatrones(C,I,J,StartsAtX,StartsAtY,Direction,Pattern) &
								// Aqui se crea el patron de figura "Pattern" a explotar 
								not Pattern = "none" & 
								posicionesIntercambiadas(X1,Y1,X2,Y2)
<-
	!handlePattern(C,StartsAtX,StartsAtY,Direction,Pattern); // Realiza la explosión de la figura y su propagación
...
/*
	...
	Añadir los nuevos patrones que se originan como consecuencia de los intercambios
	entre fichas especiales
*/
...
// la explosion es directamente realizada por el movimiento del jugador
+!setMovedSteak(X,Y,Pattern) : 
	explosionFlag(1) & tablero(celda(X,Y,Own),e) & specialSteak(Pattern,Type)<-
...
+!generateSpecialSteak(Pattern,Color) : 
	explosionFlag(1) & tablero(celda(X,Y,Own),m) & specialSteak(Pattern,Type)
<- //La explosion se produjo directamente por el movimiento del jugador, y la ficha especial se debe de colocar en la ficha movida.
			put(Color,X,Y,Type);
			-tablero(celda(X,Y,Own),m);
...
+!generateSpecialSteak(Pattern,Color) : 
	explosionFlag(0) & tablero(celda(X,Y,Own),e) & specialSteak(Pattern,Type)
<-  //La explosion se produjo de forma indirecta por la caida de fichas. La ficha especial se coloca de forma aleatoria por unificacion.
			put(Color,X,Y,Type);
			-tablero(celda(X,Y,Own),e);
			+tablero(celda(X,Y,Own),ficha(Color,Type));
			!specialStickGenerationPoints(Type).
...
+!explosion(X,Y) : tablero(celda(X,Y,Own),ficha(C,T)) & direction(D) & turnoActual(A) & level(L) & points(L,A,P)<-
				// Primero se elimina la ficha seleccionada, luego las otras fichas afectadas en caso de ser necesario
				-tablero(celda(X,Y,Own),_); 
				// Se indica que la celda está vacia
				+tablero(celda(X,Y,Own),e);
				// Se cambia el propietario de ser preciso
				!changeOwner(X,Y);
				// Se realiza la eliminación en el tablero
				delete(C,X,Y);
				// Se propaga la eliminación acorde a las fichas a borrar
				!specialExplosion(X,Y,C,T,D,A,P,L).

//Explosion en una celda vacia
+!explosion(X,Y) : tablero(celda(X,Y,_),e). 

//Gestión de explosiones inesperadas
...
La intención specialExplosion(X,Y,C,T,D,A,P,L) gestiona la propagación de la explosión 
en función del tipo de ficha "T" de color "C" que haya en la "pos(X,Y)" y 
de la dirección "D" del movimiento
Las variables "A", "P" y "L" sirven para 
contabilizar los puntos "P" que incrementa el jugador "A" en el nivel "L"
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = ip  & (D="up" | D="down")<-     
//Explosion en linea vertical
			-points(L,A,P);
			+points(L,A,P+2);
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = ip  & (D="left" | D="right")<-     
//Explosion en linea horizontal
			-points(L,A,P);
			+points(L,A,P+2);
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = gs & tablero(celda(NX,NY,Own),ficha(C,_)) & not X=NX & not Y=NY <-	 
//Explosion de una ficha del mismo color (distinta a si misma)
			-points(L,A,P);
			+points(L,A,P+4);
			!explosion(NX,NY);
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = co <-  			
//Explosion en cuadrado 3x3
			-points(L,A,P);
			+points(L,A,P+6);
			for ( .range(I,-1,1) ) {
				for ( .range(J,-1,1) ) {
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = ct <-     
//Explosion de todas las fichas de ese color
			-points(L,A,P);
			+points(L,A,P+8);
...
+!changeOwner(NX,NY) : 
	posicionesIntercambiadas(X,Y,_,_) & tablero(celda(X,Y,Owner),_) & 
	not Owner=0 & tablero(celda(NX,NY,_),_) 
<- //Si la celda de origen tiene Owner, las explosiones desencadenadas tendran ese Owner
